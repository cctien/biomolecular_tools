...
def extract_sequences(pdb_file: str) -> Mapping[str, str]:
    ...
    structure = biotite.structure.io.load_structure(pdb_file)
    ...
    for chain_id in get_chains(structure):
        ...
        assert sequence.is_valid(), f"Invalid sequence for chain {chain_id} in {pdb_file}"
        sequences[chain_id.item()] = "".join(sequence.symbols.tolist())
