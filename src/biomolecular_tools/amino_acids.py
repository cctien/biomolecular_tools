...
def amino_acid_code_three_to_one(three_letter_code: str) -> str | None:
    try:
        return ProteinSequence.convert_letter_3to1(three_letter_code)
    except KeyError:
        return None
...
def amino_acid_code_one_to_three(one_letter_code: str) -> str | None:
    try:
        return ProteinSequence.convert_letter_1to3(one_letter_code)
    except KeyError:
        return None
...
def _test():
    ...
    assert amino_acid_code_one_to_three("X") == "UNK"
    assert amino_acid_code_one_to_three("J") is None
